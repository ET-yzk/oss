    ...
    <c:forEach items="${pb.beanList}" var="goods">
        <tr>
            ...
            <td>
                ...
                <c:if test="${goods.kid eq '09'}">书籍类</c:if>
                <c:if test="${goods.kid eq '10'}">水果类</c:if>
                <c:if test="${goods.kid eq '00'}">不详</c:if>
            </td>
            ...
            <td>
                <a href="<c:url value='/GoodsServlet?method=preEdit&gid=${goods.gid}'/> ">编辑</a>
                <a href="<c:url value='/GoodsServlet?method=delete&gid=${goods.gid}'/> ">删除</a>
            </td>
        </tr>
    </c:forEach>
</table>
...
<center>
    ...
    <input type="button" name="Submit" onclick="window.location.href = '/goods/goods_query.jsp'" value="返回搜索">
